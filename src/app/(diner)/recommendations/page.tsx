...
export default function DinerRecommendations() {
  ...
  function ChefCard({ chef }: { chef: Chef }) {
    ...
    return (
      <div style={{ background: "#0f0f0f", border: `1px solid ${isFav ? chef.color + "44" : "#1e1e1e"}`, borderRadius: 14, overflow: "hidden", transition: "border-color 0.2s" }}>
        {/* Headshot */}
        <div style={{ height: 130, position: "relative", overflow: "hidden", background: "#141414" }}>
          ...
          <button onClick={() => toggleFavorite(chef.id)}
            ...
            style={{ position: "absolute", top: 10, right: 10, width: 32, height: 32, borderRadius: "50%", background: "rgba(10,10,10,0.7)", border: `1px solid ${isFav ? chef.color : "#27272a"}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: favorites.length >= MAX_FAVORITES && !isFav ? "not-allowed" : "pointer" }}>
            <Heart size={14} fill={isFav ? chef.color : "none"} color={isFav ? chef.color : "#71717a"} strokeWidth={1.75} />
          </button>
          {wasBooked && (
          ...
          <div style={{ position: "absolute", bottom: 10, left: 14, right: 44 }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: "#f5f0e8", fontFamily: "var(--font-playfair)", lineHeight: 1.2 }}>{chef.name}</div>
        ...
        <div style={{ padding: "12px 14px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ fontWeight: 800, color: "#C8A97E", fontSize: 15 }}>${chef.price}<span style={{ fontSize: 10, color: "#71717a", fontWeight: 400 }}>/pp</span></div>
          </div>
        </div>
  ...
  return (
    <div style={{ maxWidth: 860 }}>
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 24, borderBottom: "1px solid #1e1e1e" }}>
        {([
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            style={{ padding: "10px 18px", background: "transparent", border: "none", borderBottom: activeTab === tab.id ? "2px solid #C8A97E" : "2px solid transparent", color: activeTab === tab.id ? "#f5f0e8" : "#52525b", fontSize: 13, fontWeight: activeTab === tab.id ? 700 : 500, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.15s", marginBottom: -1 }}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {activeTab === "favorites" && favorites.length === 0 && (
        <div style={{ textAlign: "center", padding: "56px 0", color: "#52525b" }}>
          <Heart size={32} color="#27272a" strokeWidth={1.5} style={{ margin: "0 auto 12px", display: "block" }} />
          <div style={{ fontSize: 14, color: "#71717a", marginBottom: 6 }}>No favorites yet</div>
