...
export default function DinerBookings() {
  ...
  const inputStyle: React.CSSProperties = {
    width: "100%", background: "var(--bg-tertiary)", border: "1px solid var(--border-mid)", borderRadius: 10,
    ...
    <div style={{ maxWidth: 900, fontFamily: "'DM Sans', sans-serif", margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 18, marginBottom: 32 }}>
        ...
        <div>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 900, color: "var(--text-primary)", fontFamily: "var(--font-playfair)", margin: "0 0 4px" }}>
      ...
      {/* ── Calendar ──────────────────────────────────────────────────── */}
      <div style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden", marginBottom: 28 }}>
        {/* Month nav */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: "1px solid var(--border-subtle)" }}>
          <button onClick={prevMonth} style={{ padding: "5px 12px", borderRadius: 8, background: "transparent", border: "1px solid var(--border-mid)", color: "var(--text-secondary)", cursor: "pointer", fontSize: 14, fontFamily: "'DM Sans', sans-serif" }}>←</button>
          <span style={{ fontWeight: 800, color: "var(--text-primary)", fontSize: 15, fontFamily: "var(--font-playfair)" }}>{MONTHS[viewMonth]} {viewYear}</span>
          <button onClick={nextMonth} style={{ padding: "5px 12px", borderRadius: 8, background: "transparent", border: "1px solid var(--border-mid)", color: "var(--text-secondary)", cursor: "pointer", fontSize: 14, fontFamily: "'DM Sans', sans-serif" }}>→</button>
        </div>
        ...
        {selectedDay && (byDate[selectedDay] ?? []).length > 0 && (
          <div style={{ borderTop: "1px solid var(--border-subtle)", padding: "14px 20px", background: "var(--bg-secondary)" }}>
            ...
            {(byDate[selectedDay] ?? []).map(b => {
              ...
              return (
                <div key={b.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderRadius: 10, background: "var(--bg-tertiary)", border: `1px solid ${s.border}`, marginBottom: 8 }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        {(["all","upcoming","completed","cancelled"] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: "7px 14px", borderRadius: 99, fontSize: 12, fontWeight: 600, cursor: "pointer", border: `1px solid ${filter === f ? "#C8A97E" : "var(--border-mid)"}`, background: filter === f ? "#C8A97E18" : "transparent", color: filter === f ? "#C8A97E" : "var(--text-muted)", fontFamily: "'DM Sans', sans-serif", textTransform: "capitalize" }}>
              ...
              <div style={{ padding: "16px 20px", display: "flex", alignItems: "center", gap: 16, cursor: "pointer" }}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                    <span style={{ fontWeight: 700, color: "var(--text-primary)", fontSize: 14 }}>{booking.chefBusiness}</span>
                    ...
                    <div style={{ padding: "16px 20px", borderTop: "1px solid var(--border-subtle)", background: "var(--bg-secondary)" }}>
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                        <button onClick={() => submitReview(booking.id)} disabled={!reviewRating}
                          style={{ padding: "9px 20px", borderRadius: 10, background: reviewRating ? "#C8A97E" : "var(--border-mid)", color: reviewRating ? "var(--bg)" : "var(--text-dim)", fontWeight: 700, fontSize: 13, border: "none", cursor: reviewRating ? "pointer" : "default", fontFamily: "'DM Sans', sans-serif" }}>
                          Submit Review
                        </button>
                        <button onClick={() => setReviewingId(null)} style={{ padding: "9px 14px", borderRadius: 10, background: "transparent", border: "1px solid var(--border-mid)", color: "var(--text-muted)", fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Cancel</button>
                      </div>
                    </div>
                  )}
                  {isPaying && (
                    <div style={{ padding: "16px 20px", borderTop: "1px solid var(--border-subtle)", background: "var(--bg-secondary)" }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => submitPayment(booking.id)} style={{ padding: "10px 24px", borderRadius: 10, background: "#C8A97E", color: "var(--bg)", fontWeight: 700, fontSize: 13, border: "none", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                          Pay ${booking.total}
                        </button>
                        <button onClick={() => setPayingId(null)} style={{ padding: "10px 14px", borderRadius: 10, background: "transparent", border: "1px solid var(--border-mid)", color: "var(--text-muted)", fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Cancel</button>
                      </div>
                    </div>
                  )}
                    ...
                    <div style={{ padding: "12px 20px", borderTop: "1px solid var(--border-subtle)", background: "var(--bg)", display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {booking.status === "upcoming" && booking.paymentStatus === "pending" && (
                        <button onClick={() => { setPayingId(booking.id); setReviewingId(null); }}
                          style={{ padding: "8px 16px", borderRadius: 8, background: "#C8A97E", border: "none", color: "var(--bg)", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", gap: 6 }}>
                          <CreditCard size={13} strokeWidth={1.75} /> Pay Now
                        </button>
                      )}
                      {booking.status === "upcoming" && (
                        <button style={{ padding: "8px 16px", borderRadius: 8, background: "var(--bg-tertiary)", border: "1px solid var(--border-mid)", color: "var(--text-secondary)", fontSize: 12, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", gap: 6 }}>
